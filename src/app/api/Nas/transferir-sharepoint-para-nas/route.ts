...
export async function POST(request: NextRequest) {
  console.log('📥 [NEW API Route] Requisição POST recebida em /api/Nas/transferir-sharepoint-para-nas')
  
  try {
    const body = await request.json()
    const { sharePointUrl } = body
    
    console.log('📝 [NEW API Route] Body da requisição:', { sharePointUrl })

    if (!sharePointUrl) {
      console.log('❌ [NEW API Route] sharePointUrl não fornecido')
    ...
    console.log(`[NEW API Proxy] Transferindo arquivo do SharePoint: ${sharePointUrl}`)
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`[NEW API Proxy] Erro da API NAS (${response.status}):`, errorText)
        return NextResponse.json(
          {
            sucesso: false,
      ...
      console.log('[NEW API Proxy] Resposta da API NAS:', result)
      ...
      console.error('[NEW API Proxy] Erro ao conectar com API NAS:', fetchError)
    ...
    console.error('[NEW API Proxy] Erro interno:', error)
    return NextResponse.json(

...
export function useAnswersDashboard() {
  ...
  const dashboardQuery = useQuery({
    queryKey: ['dashboard-values', searchParams],
    queryFn: () =>
      npsService.getClassificationsDashboardValues(
  ...
  const subclassQuery = useQuery({
    queryKey: ['subclassification-values', searchParams],
    queryFn: () =>
      npsService.getSubclassificationsDashboardValues(
        searchParams!.start, searchParams!.end, Number(searchParams!.unidade),
      ),
    enabled: !!searchParams,
    refetchOnWindowFocus: false,
  ...
  const handleCaptureClick = useCallback(async () => {
    ...
    downloadjs(dataURL, 'dashboard-nps.jpg', 'image/jpg')
  }, [])

...
import type { IRatingTratamento, NpsColumn, Order } from '@/types/nps'
import { renderUnidadeName, renderClassification, renderSubclassification } from '../npsHelpers'
...
interface ColumnDeps {
  data: IRatingTratamento[]
  selected: IRatingTratamento[]
  order: Order
  orderBy: keyof IRatingTratamento
  isSelected: (item: IRatingTratamento) => boolean
  isCheckedAll: (data: IRatingTratamento[]) => boolean
  handleSelected: (item: IRatingTratamento) => void
  handleSelectedAll: (checked: boolean, data: IRatingTratamento[]) => void
  handleRequestSort: (property: keyof IRatingTratamento) => void
  handleOpenModal: (type: 'classification' | 'answer' | 'answer72h', data: IRatingTratamento) => void
}

export function useTratamentoListColumns(deps: ColumnDeps): NpsColumn<IRatingTratamento>[] {
  const { data, selected, isSelected, isCheckedAll, handleSelected, handleSelectedAll, handleOpenModal } = deps

  return useMemo<NpsColumn<IRatingTratamento>[]>(() => [
    {
    ...
    ...((['quest1', 'quest2', 'quest3', 'quest4', 'quest5'] as const).map((q, i) => ({
      id: q, label: ['Auxílio navegação', 'Equipe multidisciplinar', 'Orientações', 'T. Espera', 'Atend. Enfermagem'][i],
      align: 'center' as const, minWidth: 130,
      renderValue: (row: IRatingTratamento) => renderNpsIcon(row[q]),
    }))),
    {
      id: 'nps', label: 'NPS', align: 'center', minWidth: 80,
      renderValue: (row) => renderNpsIcon(row.quest6),
    },
    {
      id: 'quest7', label: 'Comentário', align: 'center', minWidth: 300, maxWidth: 450,
      renderValue: (row) => (

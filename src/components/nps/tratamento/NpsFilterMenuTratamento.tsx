...
function NotaSubFilter({ selected, onChangeFilter, onSetAll }: { selected: string[]; onChangeFilter: (n: string, v: string, c: boolean) => void; onSetAll: <T>(q: IOptionsFilterTratamentoQuests, c: boolean, arr: T[]) => void }) {
  return (
    <>
      ...
      <CheckItem label={<span className="flex items-center gap-2"><Frown size={15} color="rgba(255,25,1,1)" /> Detrator (0-6)</span>} checked={selected.includes('Detrator')} onChange={(c) => onChangeFilter('nota', 'Detrator', c)} />
      <CheckItem label={<span className="flex items-center gap-2"><Annoyed size={15} color="rgba(200,170,0,1)" /> Neutro (7-8)</span>} checked={selected.includes('Neutro')} onChange={(c) => onChangeFilter('nota', 'Neutro', c)} />
      <CheckItem label={<span className="flex items-center gap-2"><Smile size={15} color="rgba(34,197,94,1)" /> Promotor (9-10)</span>} checked={selected.includes('Promotor')} onChange={(c) => onChangeFilter('nota', 'Promotor', c)} />
    </>
  )
}
